...
class Features():
    ...
    def import_vizwiz_image(self):
        ...
        self.vizwiz_features_test_image   = h5py.File("./nn_features_images/vizwiz_image_feature_test.hdf5", 'r')
    ...
    def import_vqa_image(self):
        ...
        self.vqa_features_val_image       = h5py.File("./nn_features_images/vqa_image_feature_val.hdf5", 'r')
        self.vqa_features_test_image      = h5py.File("./nn_features_images/vqa_image_feature_test.hdf5", 'r')
    ...
    def concat_image_features(self):
        ...
        if self.dataset == 'vizwiz':
            ...
            test  = self.vizwiz_features_test_image 
        else:
            train = self.vqa_features_train_image
            val   = self.vqa_features_val_image
            test  = self.vqa_features_test_image 
        ...
        df_test = copy.deepcopy(question_features_test_df)
        df_test['image_features'] = ""
        ...
        for i in range(len(df_test)):
            row = df_test.iloc[i,:]
            qid = str(row['QID'])
            df_test.iloc[i, len(df_test.columns)-1] = val[qid]

        self.image_features_df_train = df_train
        self.image_features_df_val   = df_val
        self.image_features_df_test  = df_test

        return df_train['image_features'].values, df_val['image_features'].values, df_test['image_features'].values
    ...
    def set_features(self, feature_columns):
        ...
        X_test  = self.preprocess_text(self.question_features_test_df[feature_columns])
        self.question_features_train = self.remove_stop_words(X_train)
        self.question_features_val   = self.remove_stop_words(X_val)
        self.question_features_test  = self.remove_stop_words(X_test)


    def create_question_feature_df(self):
        self.set_question_features = True
        if self.dataset == 'vizwiz':
            ...
            self.question_features_test_df = self.join_question_feature_target(self.vizwiz_features_test_text, self.vizwiz_features_test_color, 
            ...
            self.question_features_test_df = self.join_question_feature_target(self.vqa_features_test_text, self.vqa_features_test_color,
                self.vqa_targets_test)


    def set_targets(self):

        if self.set_question_features:
            ...
            self.txt_test  = self.question_features_test_df['TXT'].values.astype('float32')
            self.col_test  = self.question_features_test_df['COL'].values.astype('float32')
            self.cnt_test  = self.question_features_test_df['CNT'].values.astype('float32')
    ...
    def get_word_embedding(self):
        ...
        test_seq    = tok.texts_to_sequences(self.question_features_test)
        ...
        # move to local gensim dir & unzip
        gensim_dir = '/home/skillPredict/.local/lib/python3.6/site-packages/gensim/test/test_data/'
        googlenews_corpus = 'GoogleNews-vectors-negative300.bin'
        googlenews_corpus_path = os.path.join(gensim_dir, googlenews_corpus)

        logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
        word2vec_model = gensim.models.KeyedVectors.load_word2vec_format(datapath(googlenews_corpus_path),
                                                                 binary=True)
        # get word vetors
        embeddings_index = {}
