...
def ocr_request(vision_base_url, key, image_url):
...
def recognize_handwritten_text(vision_base_url, key, image_url):
    """ return a list of lowercase strings, may contain special characters """
...
def write_to_file(vision_base_url, key, df, output_file_path, dataset):
    ...
    image_url_base = os.path.abspath(f'../../VQA_data/image/{dataset}')
    assert(len(df) > 0)
    ...
    for i, row in df.iterrows():
        ...
        try:
            ocr_text   = ocr_request(vision_base_url, key, image_url)
            handwritten_text = recognize_handwritten_text(vision_base_url, key, image_url)
