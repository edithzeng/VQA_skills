...
def preprocess(dataset, features=['QSN','descriptions','tags','dominant_colors','handwritten_text','ocr_text'], n_classes=3, skill=None, verbose=True, pretrained_embedding=False):
    ...
    if n_classes > 3:
        raise ValueError("Number of class cannot be greater than 3")
    ...
    # combine labels for the training split
    if n_classes == 1:
    ...
    # combine labels for the val split
    if n_classes == 1:
    ...
    MAX_DOC_LEN = 40
    if verbose:
        print("Performing PCA with {} eigenvectors".format(MAX_DOC_LEN))
