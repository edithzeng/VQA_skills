...
class Features():
    ...
    def __init__(self, dataset, src='image_features', target='../data/three_vote_threshold'):
    ...
    def import_vizwiz_text(self):
        self.vizwiz_features_train_color = pd.read_csv(os.path.join(self.src, 'data/vizwiz_train_color_recognition.csv'),
        ...
        self.vizwiz_features_train_text = pd.read_csv(os.path.join(self.src, 'data/vizwiz_train_text_recognition.csv'),
                                      delimiter=';', engine='python', 
                                      dtype={'qid':str, 'question':str, 'descriptions':list, 'ocr_text':list, 'handwritten_text':list},
                                      quotechar='"', error_bad_lines=False, warn_bad_lines=False)
        self.vizwiz_features_val_color = pd.read_csv(os.path.join(self.src, 'data/vizwiz_val_color_recognition.csv'),
                                        delimiter=';', engine='python',
                                        dtype={'qid':str, 'question':str, 'descriptions':list, 'tags':list, 'dominant_colors':list},
                                        quotechar='"', error_bad_lines=False, warn_bad_lines=False)
        self.vizwiz_features_val_text = pd.read_csv(os.path.join(self.src, 'data/vizwiz_val_text_recognition.csv'),
                                      delimiter=';', engine='python', 
                                      dtype={'qid':str, 'question':str, 'descriptions':list, 'ocr_text':list, 'handwritten_text':list},
                                      quotechar='"', error_bad_lines=False, warn_bad_lines=False)
        self.vizwiz_targets_train = pd.read_csv(os.path.join(self.target, 'vizwiz_skill_typ_train.csv'), dtype={'QID':str},
                                        delimiter=',', quotechar='"',
                                        engine='python', 
                                        error_bad_lines=False, warn_bad_lines=False)
        self.vizwiz_targets_val = pd.read_csv(os.path.join(self.target, 'vizwiz_skill_typ_val.csv)', 
                                    dtype={'QID':str}, delimiter=',', quotechar='"', engine='python', 
                                    error_bad_lines=False, warn_bad_lines=False)

    def import_vizwiz_image(self):
        """ import visual feature extracted w/ resnet """
    ...
    def import_vqa_text(self):
        """ import text/question based data """
        self.vqa_features_train_color = pd.read_csv(os.path.join(self.src, 'data/vqa_train_color_recognition.csv'),
                                    delimiter=';', engine='python', 
                                    dtype={'qid':str, 'question':str, 'descriptions':list, 'tags':list, 'dominant_colors':list},
                                    quotechar='"', error_bad_lines=False, warn_bad_lines=False)

        self.vqa_features_train_text = pd.read_csv(os.path.join(self.src, 'data/vqa_train_text_recognition.csv'),
                                      delimiter=';', engine='python', encoding='latin-1',
                                      dtype={'qid':str, 'question':str, 'ocr_text':list, 'handwritten_text':list},
                                      quotechar='"', error_bad_lines=False, warn_bad_lines=False)

        self.vqa_features_val_color = pd.read_csv(os.path.join(self.src, 'data/vqa_val_color_recognition.csv'),
                                    delimiter=';', engine='python',
                                        dtype={'qid':str, 'question':str, 'descriptions':list,
                                            'tags':list, 'dominant_colors':list},
                                        quotechar='"', error_bad_lines=False, warn_bad_lines=False)

        self.vqa_features_val_text = pd.read_csv(os.path.join(self.src, 'data/vqa_val_text_recognition.csv'),
                                      delimiter=';', engine='python', 
                                      dtype={'qid':str, 'question':str, 'ocr_text':list, 'handwritten_text':list},
                                      quotechar='"', error_bad_lines=False, warn_bad_lines=False)
    ...
    def import_vqa_image(self):
        self.vqa_features_train_image = # pd.read_csv("./nn_features_images/steady/vqa")
    ...
    def join_question_feature_target(self, feature_df_text, feature_df_color, target_df):
        """ join question-based feature with the target (binary flag for each class 
            returns the modified df with correct target values
        """
    ...
    def concat_image_features(self):
        """ Concatenate features extracted from the image """
    ...
    def lem(self, s):
        """ lemmatize a string (multiple words) """
        arr = s.split(" ")
    ...
    def get_word_embedding(self, pretrained_embedding):
        """ get word embedding for all texts 
        ...
        if pretrained_embedding:
            if not os.path.exists(pretrained_embedding):
                raise FileNoeFoundError(pretrained_embedding)
            # load pre-trained word2vec on GoogleNews (https://code.google.com/archive/p/word2vec/)
            logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
            word2vec_model = gensim.models.KeyedVectors.load_word2vec_format(datapath(pretrained_embedding), binary=True)
        else: # use the specified embedding dimension
            logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
